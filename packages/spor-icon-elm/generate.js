...
async function generateSvgs() {
    ...
    await Promise.all(svgs).then(svgs => {
        ...
        // Autogenerated elm svgs not to be exposed to end-user. To be appended to bottom of elm-filme and used in toHtml.
        ...
        const uniqueNames = Object.keys(groupBy(gatheredSvgs, "name")).map(camelCase)

        const moduleHeader = generateModuleHeader("Icons", uniqueNames.sort(), uniqueIconSizes);
...
function generateBasicCase(iconName) {
return`
{-| -}
...
${iconName} iconConfig =
    ...
    case iconConfig.variant of
        Fill ->
            ${iconName + "Fill" + "_"} (attribute "fill" iconConfig.color) :: attributes
        
        Stroke ->
            ${iconName + "Outline" + "_"} (attribute "stroke" iconConfig.color) :: attributes
...
${svgNames.map(svgName => generateBasicCase(svgName)).join("\n\n")}
`;
}
