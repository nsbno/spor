import { promises as fs } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

async function getAllJsonFiles(directories: string): Promise<string[]> {
  const dirents = await fs.readdir(directories, { withFileTypes: true });
  const files = await Promise.all(
    dirents.map((dirent) => {
      const result = path.resolve(directories, dirent.name);
      if (dirent.isDirectory()) {
        return getAllJsonFiles(result);
      } else if (dirent.isFile() && result.endsWith(".json")) {
        return [result];
      } else {
        return [];
      }
    }),
  );
  return files.flat();
}

function setNested(
  object: Record<string, unknown>,
  keys: string[],
  value: unknown,
) {
  let current: Record<string, unknown> = object;
  for (const [index, key] of keys.entries()) {
    if (index === keys.length - 1) {
      current[key] = value;
    } else {
      if (!current[key]) current[key] = {};
      current = current[key] as Record<string, unknown>;
    }
  }
}

export async function generateTokenModuleFromJson() {
  try {
    const tokensDirectory = path.resolve(__dirname, "../tokens");
    const outputDirectory = path.resolve(__dirname, "../dist/raw-tokens");
    const outputJsPath = path.join(outputDirectory, "index.js");
    const outputDtsPath = path.join(outputDirectory, "index.d.ts");

    await fs.mkdir(outputDirectory, { recursive: true });

    const jsonFiles = await getAllJsonFiles(tokensDirectory);

    const tokens: Record<string, unknown> = {};

    for (const file of jsonFiles) {
      const releativePath = path
        .relative(tokensDirectory, file)
        .replace(/\.json$/, "");
      const keys = releativePath.split(path.sep);
      const content = await fs.readFile(file, "utf8");
      setNested(tokens, keys, JSON.parse(content));
    }

    const jsContent = `const tokens = ${JSON.stringify(
      tokens,
      null,
      2,
    )};\nexport default tokens;\n`;

    const dtsContent = `declare const tokens: ${JSON.stringify(
      tokens,
      null,
      2,
    )};\nexport default tokens;\n`;

    await fs.writeFile(outputJsPath, jsContent, "utf8");
    await fs.writeFile(outputDtsPath, dtsContent, "utf8");
    console.log(
      `Successfully converted all JSON tokens to JS and type declarations at ${outputDirectory}`,
    );
  } catch (error) {
    console.error("Error converting JSON to JS/TypeScript:", error);
    throw error;
  }
}
