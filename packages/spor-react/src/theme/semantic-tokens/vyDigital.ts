export const vyDigital = {
  bg: {
    DEFAULT: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.night}",
      },
    },
    secondary: {
      value: {
        _light: "{colors.lightGrey}",
        _dark: "{colors.night}",
      },
    },
    tertiary: {
      value: {
        _light: "{colors.mint}",
        _dark: "{colors.darkTeal}",
      },
    },
  },
  text: {
    DEFAULT: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
    secondary: {
      value: {
        _light: "{colors.darkTeal}",
        _dark: "{colors.seaMist}",
      },
    },
    tertiary: {
      value: {
        _light: "{colors.dimGrey}",
        _dark: "{colors.whiteAlpha.700}",
      },
    },
    inverted: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.darkTeal}",
      },
    },
    highlight: {
      value: {
        _light: "{colors.pine}",
        _dark: "{colors.blueGreen}",
      },
    },
    disabled: {
      value: {
        _light: "{colors.blackAlpha.400}",
        _dark: "{colors.whiteAlpha.400}",
      },
    },
  },
  icon: {
    DEFAULT: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
    secondary: {
      value: {
        _light: "{colors.darkTeal}",
        _dark: "{colors.seaMist}",
      },
    },
    inverted: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.darkTeal}",
      },
    },
    highlight: {
      value: {
        _light: "{colors.pine}",
        _dark: "{colors.blueGreen}",
      },
    },
    disabled: {
      value: {
        _light: "{colors.blackAlpha.200}",
        _dark: "{colors.whiteAlpha.300}",
      },
    },
  },
  outline: {
    DEFAULT: {
      value: {
        _light: "{colors.blackAlpha.400}",
        _dark: "{colors.whiteAlpha.400}",
      },
    },
    inverted: {
      value: {
        _light: "{colors.whiteAlpha.400}",
        _dark: "{colors.blackAlpha.400}",
      },
    },
    focus: {
      value: {
        _light: "{colors.greenHaze}",
        _dark: "{colors.azure}",
      },
    },
    error: {
      value: {
        _light: "{colors.brightRed}",
        _dark: "{colors.brightRed}",
      },
    },
    disabled: {
      value: {
        _light: "{colors.blackAlpha.200}",
        _dark: "{colors.whiteAlpha.200}",
      },
    },
  },
  surface: {
    DEFAULT: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.night}",
      },
    },
    secondary: {
      value: {
        _light: "{colors.seaMist}",
        _dark: "{colors.darkTeal}",
      },
    },
    tertiary: {
      value: {
        _light: "{colors.darkTeal}",
        _dark: "{colors.pine}",
      },
    },
    disabled: {
      value: {
        _light: "{colors.blackAlpha.100}",
        _dark: "{colors.whiteAlpha.100}",
      },
    },
  },
  core: {
    surface: {
      active: {
        value: {
          _light: "{colors.mint}",
          _dark: "{colors.whiteAlpha.100}",
        },
      },
    },
    outline: {
      DEFAULT: {
        value: {
          _light: "{colors.blackAlpha.400}",
          _dark: "{colors.whiteAlpha.400}",
        },
      },
      hover: {
        value: {
          _light: "{colors.darkGrey}",
          _dark: "{colors.white}",
        },
      },
    },
    text: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
    icon: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
  },
  brand: {
    surface: {
      DEFAULT: {
        value: {
          _light: "{colors.pine}",
          _dark: "{colors.coralGreen}",
        },
      },
      hover: {
        value: {
          _light: "{colors.darkTeal}",
          _dark: "{colors.blueGreen}",
        },
      },
      active: {
        value: {
          _light: "{colors.greenHaze}",
          _dark: "{colors.seaMist}",
        },
      },
    },
    text: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.darkTeal}",
      },
    },
    icon: {
      value: {
        _light: "{colors.white}",
        _dark: "{colors.darkTeal}",
      },
    },
  },
  accent: {
    surface: {
      DEFAULT: {
        value: {
          _light: "{colors.seaMist}",
          _dark: "{colors.primaryGreen}",
        },
      },
      hover: {
        value: {
          _light: "{colors.coralGreen}",
          _dark: "{colors.greenHaze}",
        },
      },
      active: {
        value: {
          _light: "{colors.mint}",
          _dark: "{colors.darkTeal}",
        },
      },
    },
    text: {
      value: {
        _light: "{colors.darkTeal}",
        _dark: "{colors.seaMist}",
      },
    },
    icon: {
      value: {
        _light: "{colors.darkTeal}",
        _dark: "{colors.seaMist}",
      },
    },
    bg: {
      value: {
        _light: "{colors.mint}",
        _dark: "{colors.darkTeal}",
      },
    },
  },
  floating: {
    surface: {
      DEFAULT: {
        value: {
          _light: "{colors.white}",
          _dark: "{colors.whiteAlpha.100}",
        },
      },
      hover: {
        value: {
          _light: "{colors.white}",
          _dark: "{colors.whiteAlpha.200}",
        },
      },
      active: {
        value: {
          _light: "{colors.mint}",
          _dark: "{colors.night}",
        },
      },
    },
    outline: {
      DEFAULT: {
        value: {
          _light: "{colors.platinum}",
          _dark: "{colors.whiteAlpha.300}",
        },
      },
      hover: {
        value: {
          _light: "{colors.steel}",
          _dark: "{colors.white}",
        },
      },
      active: {
        value: {
          _light: "{colors.silver}",
          _dark: "{colors.white}",
        },
      },
    },
    text: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
    icon: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
  },
  ghost: {
    surface: {
      hover: {
        value: {
          _light: "{colors.seaMist}",
          _dark: "{colors.whiteAlpha.200}",
        },
      },
      active: {
        value: {
          _light: "{colors.mint}",
          _dark: "{colors.whiteAlpha.100}",
        },
      },
    },
    text: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
    icon: {
      value: {
        _light: "{colors.darkGrey}",
        _dark: "{colors.white}",
      },
    },
  },
};
