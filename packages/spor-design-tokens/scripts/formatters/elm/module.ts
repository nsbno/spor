...
export const elmFormatter: Named<Format> = {
  ...
  formatter: function ({ dictionary, file }) {
    ...
    const exposing = moduleType
      .exposings()
      .concat(
        dictionary.allProperties.map((prop) => prop.name.replace(".", "_")),
      )
      .join(", ");

    const definitions = moduleType
      .definitions()
      .concat(
...
class ModuleType {
  ...
  exposings(): Array<string> {
    return [this.name, this.unwrapperName()];
  }
  ...
  generateElmConstant(token: TransformedToken): Array<string> {
    return [
      ...
      `${defaultIndentation}${this.name} <| ${this.wrappedType.construct(
        token.value,
      )}`,
      "",
      "",
...
const shadowTypeConstruction: ModuleWrappedType = {
  ...
  construct(input: string): string {
    ...
    switch (args.length) {
      case 4:
        return `Css.boxShadow4 ${args.join(" ")}`;
      case 5:
        return `Css.boxShadow5 ${args.join(" ")}`;
      default:
        throw new Error(
          `Don\'t know how to handle this box-shadow value: ${input}`,
        );
    }
  },
};
...
function generateModuleType(options: any): ModuleType {
  const wrappedType =
    moduleTypeInnerType.get(options.type) || stringTypeConstruction;
