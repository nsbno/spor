...
const INTERNAL_URLS_REGEX = /^https:\/\/(design\.)?vy\.no/;
...
export function Link({ to, href, ...props }: LinkProps) {
  ...
  if (typeof destination === "string") {
    if (
      EXTERNAL_URL_REGEX.test(destination) &&
      !INTERNAL_URLS_REGEX.test(destination)
    ) {
      return (
  ...
  return (
    <RemixLink prefetch="intent" to={destination} viewTransition {...props} />
  );
}
